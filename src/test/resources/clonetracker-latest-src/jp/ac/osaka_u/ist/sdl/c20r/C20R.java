...
import jp.ac.osaka_u.ist.sdl.c20r.genealogy.writer.GenealogyWriter;
...
public class C20R {
	...
	public static void main(final String[] args) {
		...
		{
			final Option crd = new Option("crd", "crdmode", true, "mode of crd");
			crd.setRequired(false);
			crd.setArgs(1);
			options.addOption(crd);
		}
	...
	private void trace() {
		...
		final int START_REV = CONFIG.getSTART_REVISION();
		final int END_REV = CONFIG.getEND_REVISION();
		final CRDMode CRD_MODE = CONFIG.getCRD_MODE();

		final Tracer tracer = new Tracer(DB_LOCATION, OUTPUT_DIR, WORKING_DIR,
				THREADS_COUNT, START_REV, END_REV, CRD_MODE);
		tracer.trace();
	}
	...
	private void sequentialAnalysis() {
		...
		final ClonePairDetectorWithDB pairDetector = new ClonePairDetectorWithDB(
				START_REV, END_REV, THREADS_COUNT, DB_LOCATION, CRD_MODE);
		pairDetector.run();
	...
	private void createGenealogyGraph() {
		...
		final GenealogyWriter writer = new GenealogyWriter(OUTPUT_DIR,
				DB_LOCATION);
		writer.write();
	}
