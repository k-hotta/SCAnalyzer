...
public class TraceThread implements Runnable {
	...
	public void run() {
		while (true) {
			...
			System.out.println("\tnow analyzing revisions " + beforeRevisionNum + " and "
					+ afterRevisionNum + ". [" + (currentIndex + 1)
					+ "/" + revisions.length + "]");
			...
			final String resultStr = ResultStringCreator.create(retriever
					.getRevisionManager(), (t2 - t1) / 1000 / 1000);
			
			final long t3 = System.nanoTime();

			//ResultWriter.getInstance().pool(beforeRevisionNum,
					//afterRevisionNum, resultStr);
			ResultWriter.getInstance().write(beforeRevisionNum, afterRevisionNum, resultStr);
