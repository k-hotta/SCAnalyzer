...
public class TracerMain {
	...
	public static void main(String[] args) {
		...
		Retriever retriever = new Retriever(beforeRevisionNum, afterRevisionNum);
		retriever.retrieve();
		
		RevisionManager manager = retriever.getRevisionManager();
		
		final String resultStr = ResultStringCreator.create(manager, 0);
		ResultWriter.getInstance().pool(beforeRevisionNum, afterRevisionNum, resultStr);
		ResultWriter.getInstance().writeAll();
		
		DBConnection.getInstance().close();
		
		int matchedCount = manager.getCloneManager().getMatchedCloneSetPairs().size();
		int deletedCount = manager.getCloneManager().getDeletedCloneSetPairs().size();
		int addedCount = manager.getCloneManager().getAddedCloneSetPairs().size();
		...
		for (MatchedCloneSetPairInfo pair : manager.getCloneManager().getMatchedCloneSetPairs()) {
